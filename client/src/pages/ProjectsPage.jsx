    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex flex-col gap-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <h1 className="text-4xl font-black mb-2" style={{ color: "var(--foreground)" }}>Projets Récents</h1>
            <p style={{ color: "var(--foreground)", opacity: 0.6 }}>Découvrez les dernières opportunités et trouvez votre prochain défi.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, i) => (
            <motion.div key={project.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
              <Card className="h-full flex flex-col p-6 cursor-pointer">
                <div className="flex justify-between items-start mb-4">
                  <Badge variant="primary">{project.type}</Badge>
                  <span className="text-xs flex items-center gap-1" style={{ color: "var(--foreground)", opacity: 0.4 }}>
                    <HiClock /> {project.time}
                  </span>
                </div>
                <h3 className="text-lg font-bold mb-2 transition-colors" style={{ color: "var(--foreground)" }}>{project.title}</h3>
                <p className="text-sm mb-6 flex items-center gap-2" style={{ color: "var(--foreground)", opacity: 0.6 }}>
                  <HiBriefcase style={{ color: "#0EA5E9" }} /> Client: <span className="font-medium" style={{ color: "var(--foreground)" }}>{project.client}</span>
                </p>
                <div className="flex flex-wrap gap-2 mb-8">
                  {project.skills.map(skill => <Badge key={skill} variant="gray" className="text-xs py-0.5 px-2">{skill}</Badge>)}
                </div>
                <div className="mt-auto pt-4 flex justify-between items-center" style={{ borderTop: "1px solid var(--border)" }}>
                  <div>
                    <p className="text-xs font-medium" style={{ color: "var(--foreground)", opacity: 0.4 }}>Budget</p>
                    <p className="text-lg font-black" style={{ color: "#0EA5E9" }}>{project.budget}</p>
                  </div>
                  <Button size="sm" variant="primary">Postuler</Button>
                </div>
              </Card>
            </motion.div>
          ))}
